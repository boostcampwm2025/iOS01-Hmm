...
final class DevBalanceRepository: BalanceRepository {
    ...
    func uploadPolicy(tab: Tab, data: PolicyDTO) async throws {
        ...
        let versionDoc = dataBase.collection(tab.rawValue).document(
            data.version
        )
        ...
        print("✅ [Dev] Policy \(data.version) 업로드 완료 (Tab: \(tab))")
    }

    func fetchVersionList(tab: Tab) async throws -> [String] {
        let snapshot = try await dataBase.collection(tab.rawValue).getDocuments()
    ...
    func fetchActiveVersion(for tab: Tab) async throws -> String? {
        let doc = try await dataBase.collection(Tab.version.rawValue).document(tab.rawValue).getDocument()
        return doc.data()?["version"] as? String
    }

    func setActiveVersion(for tab: Tab, version: String) async throws {
        try await dataBase
            .collection(Tab.version.rawValue)
            .document(tab.rawValue)
            .setData([
            "version": version,
